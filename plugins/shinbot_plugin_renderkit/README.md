...
RenderKit turns standard HTML/CSS or SVG into image files that other plugins can
send as `img` message elements. It is not tied to a specific business domain. A
plugin such as minesweeper can generate its own template and use RenderKit when
it is installed, while keeping a text fallback when it is not installed.
...
can inject backend objects for deterministic rendering.

## Scope

RenderKit accepts existing standards: HTML, CSS, SVG, and Jinja2 templates. It
does not define a custom scene format and does not understand plugin-specific
concepts such as game boards, mines, score tables, or alerts.
